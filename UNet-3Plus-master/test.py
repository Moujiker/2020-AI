...
def parse_args():
    ...
    parser.add_argument('--name', default='LiTS_UNet_3Plus_woDS', help='model name')
...
def GetPatchPosition(PatchPath):
    ...
    firstName = npName
    overNum = npName.find(".npy")  #返回索引
    npName = npName[0:overNum]
    PeopleName = npName
    overNum = npName.find("_")
    while(overNum != -1):
        npName = npName[overNum+1:len(npName)]   #slice的数字
        overNum = npName.find("_")
    ...
    '''
                ...
                for i in range(output.shape[0]):
                    ...
                    if (startFlag == 1):  #第一个块的处理
                        ...
                        OnePeople = np.zeros([155, 512, 512], dtype=np.uint8)
                        OneMask = np.zeros([155, 512, 512], dtype=np.uint8)
                        # 创建三个全黑的三维矩阵，分别用于预测出来的WT、TC、ET分块的拼接
                        OneWT = np.zeros([155, 512, 512], dtype=np.uint8)
                        OneTC = np.zeros([155, 512, 512], dtype=np.uint8)
                        OneET = np.zeros([155, 512, 512], dtype=np.uint8)
                        # 创建三个全黑的三维矩阵，分别用于真实的WT、TC、ET分块的拼接
                        OneWTMask = np.zeros([155, 512, 512], dtype=np.uint8)
                        OneTCMask = np.zeros([155, 512, 512], dtype=np.uint8)
                        OneETMask = np.zeros([155, 512, 512], dtype=np.uint8)
                        # 处理预测后的分块
                        # (2, 3, 512, 512) output
                        # 预测切片的拼接
